# -*- coding: utf-8 -*-
# Usage :
# you Create a templates/django_social_share/templatetags/post_to_facebook.html,post_to_messenger.html ...  etc wich will  contain template of sharing
# You call template tags  : {%  load chipop_social_share  %
# You load tag :
"""""""""""
{% post_to_facebook article "" %}
{% post_to_messenger article "" %}
{% post_to_whatsapp article "" %}
...
# Messenger Docs :
# Desktop:  https://developers.facebook.com/docs/sharing/reference/send-dialog#examples
# Mobile on web browser : https://developers.facebook.com/docs/sharing/messenger/web/
FACEBOOK_APP_ID = settings.FACEBOOK_APP_ID
MESSENGER_ENDPOINT = 'https://www.facebook.com/dialog/send?link=%s&app_id=' + FACEBOOK_APP_ID + '&redirect_uri=%s'
MESSENGER_MOBILE_ENDPOINT = 'fb-messenger://share/?link=%s&app_id=' + FACEBOOK_APP_ID

# Whatsapp doc : https://faq.whatsapp.com/en/general/26000030
...
## Chipop Shares ##
@register.simple_tag(takes_context=True)
def post_to_messenger_url(context, obj_or_url=None):
    request = context['request']
    url = _build_url(request, obj_or_url)
    if mobile(request):
        context['messenger_url'] = MESSENGER_MOBILE_ENDPOINT % urlencode(url)
    else:
        context['messenger_url'] = MESSENGER_ENDPOINT % (urlencode(url), urlencode(url))
    return context


@register.inclusion_tag('django_social_share/templatetags/post_to_messenger.html', takes_context=True)
def post_to_messenger(context, obj_or_url=None, link_text='Post to Messenger'):
    context = post_to_messenger_url(context, obj_or_url)
    context['link_text'] = link_text
    return context
